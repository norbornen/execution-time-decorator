...
describe('Timer Method Decorator Routine', () => {
  ...
  it('nano timer: when call async instace method which return an error', async () => {
    class TestUsecase {
      @timer({ hr: true })
      async test(filepath: string) {
  ...
  it('nano timer: when call correct async static method', async () => {
    class TestUsecase {
      @timer({ hr: true })
  ...
  it('nano timer: when call async method which return an error', async () => {
    class TestUsecase {
      @timer({ hr: true })
      static async test(filepath: string) {
        return fs.readFile(filepath + new Date().toISOString());
